...
class _AddCustomCartBottomSheetState extends State<AddCustomCartBottomSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final cartsListState = context.read<CartsListBloc>().state;
    final existingCarts = cartsListState.carts;
              ...
              children: [
                ...
                if (_isNewCart) ...[
                  TextFormField(
                    ...
                    validator: (value) {
                      if (_isNewCart && (value == null || value.trim().isEmpty)) {
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    items: existingCarts.map((cart) {
                      final displayName =
                      (cart.name != null && cart.name!.trim().isNotEmpty)
                          ? cart.name!
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    icon: Icon(_isNewCart ? Icons.add_rounded : Icons.add_shopping_cart_rounded),
                    onPressed: () {
                      if (_formKey.currentState!.validate()) {
                        if (_isNewCart) {
                          final newCartId =
                              DateTime.now().millisecondsSinceEpoch ~/ 1000;
                          final customName = _cartNameController.text.trim();
                          ...
                          if (existingIndex >= 0) {
                            ...
                            updatedProducts[existingIndex] = CartProductEntity(
